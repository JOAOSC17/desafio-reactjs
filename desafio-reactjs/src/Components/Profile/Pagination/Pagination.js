...
const ProfilesPagination = ({data, page, totalPages, prevPage, nextPage}) => {
    ...
    return (
        ...
        <button type="button" className={(page < totalPages)?` profiles-pagination__prev ` : ('profiles-pagination__prev disable ')} onClick={()=>nextPage()}>Next</button>
        </div>
    )
}
